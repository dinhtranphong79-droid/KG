window.addEventListener('tab.open', (e) => {
  ...
  container.innerHTML = `
    <div class="small">Hero — quản lý tướng</div>
    <input id="filterHero" placeholder="Tìm tướng theo tên" style="margin-bottom:12px;width:100%;padding:6px;border-radius:6px;border:1px solid rgba(0,0,0,0.08);"/>
    <div id="heroList"></div>
  `;
...
Để 5★ thiếu: ${res.missing_to_5===Infinity?'Không xác định':res.missing_to_5+' thẻ'}`;
  ...
  function createHeroElement(h){
    ...
    box.className="hero " + (h.system==="Lửa"?"fire":h.system==="Cung"?"cung":h.system==="Độc"?"độc":h.system==="Băng"?"băng":"exp");
    ...
    barSel.innerHTML=Array.from({length:maxBarForStar(h.current_star)+1}).map((_,i)=>`<option value="${i}" ${i===h.current_bar?'selected':''}>Bậc ${i}</option>`).join("");
  ...
  function renderSummary(){
    ...
    state.heroes.forEach(h=>{
      ...
      } else { const r=calculateUpgrade(h); let pCount=r.cards_consumed; if(r.final_star>=5) pCount+=r.cards_left; pts=pCount*CARD_POINT; totals[h.system]+=pts; }
    });
    let grandTotal=0;
